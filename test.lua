...
cmd:option('-name', 'output_mine/3d_80k_batch128_lr_10_still/imgnet', 'experiment name, snapshot prefix') 
-- cmd:option('-train', 'data_mine/ILSVRC2015_VID_test.t7', 'training data file name')
cmd:option('-train', 'data_mine/ILSVRC2015_VID_sampled_double_mid_only.t7', 'training data file name')
-- Modified on Feb 4th: set restore model
cmd:option('-restore', '', 'network snapshot file name to load')
-- cmd:option('-restore', 'output_mine/train_10_test/imgnet_003000.t7', 'network snapshot file name to load')
...
-- Modified on Feb 21th: decrease lr when plataue
local prev_dcls = 1000
local new_dcls
...
function load_model(cfg, model_path, network_filename, cuda)
  ...
  print('model (printed by test.lua):')
  for i,block in ipairs(model.input_net.modules) do
    print('input net module #' .. i)
    print(block)
  end
  ...
  -- save_obj('output_mine/tmp_test/model')
  if cuda then
...
function graph_training(cfg, model_path, snapshot_prefix, training_data_filename, network_filename)
  ...
  local batch_iterator = BatchIterator_3d.new(model, training_data)
...
-- Iterations: originally set to 50000; for feasibility testing, change to 100
  for i=1,80000 do
    if i % 5000 == 0 then
      new_dcls = mean(range(training_stats.dcls, i-4999, i))
      if prev_dcls - new_dcls < 0.01 then
        opt.lr = opt.lr / 10
    ...
    if i % 100 == 0 then
      print(string.format('iter #%d: loss: %f / lr: %f', i, loss[1], opt.lr))
    end
    ...
    if i%opt.snapshot == 0 then
      ...
      save_obj(snapshot_prefix .. '_inet.t7', model.input_net)
      save_obj(snapshot_prefix .. '_pnet.t7', model.pnet)
    end
  end
