...
cmd:option('-dataset', 'output_mine/get_prop/dataset_conv5_test.t7', 'Dataset for storing rois obtained from pnet training')
...
function get_train_proposal(cfg, model_path, training_data_filename, network_filename, dataset_save_name)
  ...
  -- create detector
  local d = Detector_2stage_test.new(model)
  local dataset = {}    
  local img_idx_map = {}
  local curr_idx = 1
...
--     if img_idx_map[img_id] ~= nil and candidates ~= nil then
