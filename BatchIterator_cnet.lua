...
local BatchIterator_cnet = torch.class('BatchIterator_cnet')
...
function BatchIterator_cnet:__init(model, training_data, pnet_output, randomize)
  local cfg = model.cfg
  ...
  -- debug
  -- save_obj('output_mine/tmp_orig/pnet', model.pnet)
  ...
  print('BatchIterator_cnet: self.training: #list=' .. #(self.training.list))
...
function BatchIterator_cnet:processImage(img, rois)
  local cfg = self.cfg
...
function BatchIterator_cnet:nextTraining(count)
  local cfg = self.cfg
  local batch = {}
  ...
  local function try_add_next()
    ...
    local gt_rois = deep_copy(self.ground_truth[fn].rois)   -- copy RoIs ground-truth data (will be manipulated)
    print('#gt_rois = ' .. #gt_rois)
    print('gt_rois')
    print(gt_rois)
    local rois = self.pnet_output[fn].rois
...
function BatchIterator_cnet:nextValidation(count)
  local cfg = self.cfg
  local batch = {}
  count = count or 1
...
function BatchIterator_cnet:getImage(set)
  local cfg = self.cfg
  local batch = {}
  succ = false
