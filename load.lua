-- Note by Bingbin: Try loading a pre-trained Caffe model
...
cmd:option('-name', 'output_mine/orig_80k_batch128_lr5_per_2k/imgnet', 'experiment name, snapshot prefix') 
...
function load_model(cfg, model_path, network_filename, cuda)
  ...
  print('model (printed by orig.lua)')
...
function evaluation_demo(cfg, model, training_data_filename)
  -- load trainnig data
...
prototxt = ''
binary = 'models/pretrained_models/ZF.caffemodel'
net = loadcaffe.load(prototxt, binary)

evaluation_demo(cfg, net, opt.name)
