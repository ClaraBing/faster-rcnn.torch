...
cmd:option('-name', 'output_mine/tmp/imgnet', 'experiment name, snapshot prefix') 
...
cmd:option('-plot', 50, 'plot training progress interval')
...
-- Iterations: originally set to 50000; for feasibility testing, change to 100
  for i=1,10000 do
    ...
    if i%opt.snapshot == 0 then
      -- save snapshot
      -- Mark save_model(string.format('%s_%06d.t7', snapshot_prefix, i), weights, opt, training_stats)
    end
  end

  -- save trained model
  -- Mark save_model(string.format('%s.t7', snapshot_prefix), weights, opt, training_stats)
