...
local Anchors_3d = torch.class('Anchors_3d')

local BIN_SIZE = 16   -- granularity for mapping center points to nearby-anchors (for nearby_aversion)

local debug = false

function Anchors_3d:__init(input_net, proposal_net, scales)
  -- create localizers
  self.localizers = {}

  -- Modified on Feb 4th: check scales
  if #scales ~= 4 then
    print('WARNING: # of scales != 4. \n Will affect Anchors_3d:findRangesXY.')
  end
  local input_node = input_net.outnode.children[1]
  for i=1,#scales do
    print('(Anchors_3d.lua) Localizer_3d init #' .. i)
    self.localizers[i] = Localizer_3d.new(input_node, proposal_net.outnode.children[i])
  end
...
function Anchors_3d:get(layer, aspect, y, x)
...
function Anchors_3d:findNearby(centerX, centerY)
  local found = {}
...
function Anchors_3d:findRangesXY(rect, clip_rect)
  -- debug
  -- print('// in findRangesXY')
  -- print('rect:')
  -- print(rect)
  -- print('clip_rect')
  -- print(clip_rect)
  ...
  -- debug
  -- print('// exiting findRangesXY: ranges = ')
  -- print(ranges)

  return ranges
end

function Anchors_3d:findPositive(roi_list, clip_rect, pos_threshold, neg_threshold, include_best)
  local matches = {}
  ...
  for i,roi in ipairs(roi_list) do
    ...
    -- debug
    -- print('find positives - roi.rect & len of ranges')
    -- print(roi.rect)
    -- print(#ranges)
    local roi_flag = true
    for j,r in ipairs(ranges) do
      ...
      for y=1,r.ys:size()[1] do
        ...
        for x=1,r.xs:size()[1] do
          ...
          -- debug
          -- if y == 1 and x == 1 then
          --   print(' findPositive (x=1, y=1): ')
          --   print(anchor_rect.index)
          -- end
...
function Anchors_3d:sampleNegative(image_rect, roi_list, neg_threshold, count)
...
function Anchors_3d.inputToAnchor(anchor, rect)
...
function Anchors_3d.anchorToInput(anchor, t)
