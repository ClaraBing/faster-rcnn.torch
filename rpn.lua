...
-- cmd:option('-model', 'models/rpn_gmod.lua', 'model factory file')
cmd:option('-model', 'models/new-zf.lua', 'model factory file')
cmd:option('-name', 'output_mine/rpn/imgnet', 'experiment name, snapshot prefix') 
...
cmd:option('-dataset', 'output_mine/tmp/dataset.obj', 'Dataset for storing rois obtained from rpn training')
...
function rpn_training(cfg, model_path, snapshot_prefix, training_data_filename, network_filename)
  ...
  local eval_objective_grad = create_objective_rpn(model, weights, gradient, batch_iterator, training_stats)
...
rpn_training(cfg, opt.model, opt.name, opt.train, opt.restore)
