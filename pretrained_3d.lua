...
require 'utilities'
require 'Anchors'
require 'BatchIterator_3d' -- need to be 3d?
require 'objective_pnet_3d'
...
cmd:option('-cfg', 'config/imagenet_3d.lua', 'configuration file')
-- cmd:option('-model', 'models/vgg_small.lua', 'model factory file')
cmd:option('-model', 'models/zf_3d.lua', 'model factory file')
-- cmd:option('-name', '/disk/bingbin/faster-rcnn-torch/imgnet', 'experiment name, snapshot prefix') 
cmd:option('-name', 'output_mine/pnet_3d/double_lr4_10k_imgnet', 'experiment name, snapshot prefix') 
...
-- cmd:option('-restore', 'output_mine/pnet/pretrain_double_imgnet_066000.t7', 'network snapshot file name to load')
...
cmd:option('-dataset', 'output_mine/tmp/dataset_double.obj', 'Dataset for storing rois obtained from pnet training')
...
function pnet_training(cfg, model_path, snapshot_prefix, training_data_filename, network_filename)
  ...
  -- create/load model
  local model, weights, gradient, training_stats = load_model_3d(cfg, model_path, network_filename, true)

  if not training_stats then
    training_stats = { pcls={}, preg={}, dcls={}, dreg={}, pfg_accu={}, pbg_accu={} }
  end
  
  local batch_iterator = BatchIterator_3d.new(model, training_data, true)
  ...
  local pcls_prev, preg_prev, pcls_curr, preg_curr = 0,0,1,1
  for i=1,100000 do
    if i % 10000 == 0 then -- and i ~= 5000 then
--       pcls_prev, preg_prev = pcls_curr, preg_curr
--       pcls_curr, preg_curr = 0,0
--       for k=#training_stats.pcls-4999, #training_stats.pcls do
--           pcls_curr = pcls_curr + training_stats.pcls[k]
--           preg_curr = preg_curr + training_stats.preg[k]
--       end
--       pcls_curr = pcls_curr / 5000
--       preg_curr = preg_curr / 5000
--       if i % 30000 == 0 or pcls_curr-pcls_prev < 0.01 or preg_curr-preg_prev < 0.01 then
        if opt.lr > 1e-08 then
          opt.lr = opt.lr / 10
